 ...
 * SEARCH INTEGRATION
 */
const searchField = document.getElementsByName("searchbar")[0];
...
/**
 * Render all weather data on HTML page
 * 
 * @param {string} lat Latitude string (e.g. "lat=51.5073219")
 * @param {string} lon Longitude string (e.g. "lon=-0.1276474")
 */
export const updateWeather = function (lat, lon) {
  // Extract numeric values from lat and lon strings
  const latitude = lat.replace("lat=", "");
  const longitude = lon.replace("lon=", "");
  ...
  fetchData(url.currentWeather(latitude, longitude), function (currentWeather) {
    ...
    fetchData(url.reverseGeo(latitude, longitude), function ([{ name, country }]) {
    ...
    fetchData(url.forecast(latitude, longitude), function (forecast) {
      ...
      for (const [index, data] of forecastList.entries()) {
        if (index > 7) break; // Show only first 8 entries (24 hours)
        ...
        windLi.innerHTML = `
          <div class="card card-sm slider-card">
            <p class="body-3">${module.getHours(dateTimeUnix, timezone)}</p>
            <img src="./assets/images/weather_icons/direction.png" width="48" height="48" loading="lazy" alt="wind direction" class="weather-icon" style="transform: rotate(${windDirection - 180}deg)">
            <p class="body-3">${Math.round(windSpeed * 3.6)}km/h</p>
          </div>
        `;
